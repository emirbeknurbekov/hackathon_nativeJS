    ...
    productsList.innerHTML += `
        <div class="card m-5" style="width: 18.5rem;">
            ...
            <div class="card-body">
                ...
                <a href="#" class="btn btn-success btn-more-info" id="more-${
                  product.id
                }">MORE</a>
                ${
...
async function moreInfoAboutProducts(e) {
  let ProductsId = e.currentTarget.id.split("-")[1];
  console.log(12);
  let res = await fetch(`${PRODUCTS_API}/${ProductsId}`);
  let ProductsObj = await res.json();
  console.log(ProductsObj);
  alert(
    `title ${ProductsObj.title}\n\nDesc: ${ProductsObj.desc}\n\nCategory: ${ProductsObj.category}\n\nPrice:${ProductsObj.price}`
  );
}
function addMoreEvent() {
  let moreInfoCardBtn = document.querySelectorAll(".btn-more-info");
  moreInfoCardBtn.forEach((btn) => {
    btn.addEventListener("click", moreInfoAboutProducts);
  });
}
